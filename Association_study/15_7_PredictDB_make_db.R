...
model_summaries <- read.table('./summary/' %&% tissue %&% '_nested_cv_chr1_model_summaries.txt',header = T, stringsAsFactors = F)
tiss_summary <- read.table('./summary/' %&% tissue %&% '_nested_cv_chr1_tissue_chr_summary.txt', header = T, stringsAsFactors = F)
  
n_samples <- tiss_summary$n_samples
  
for (i in 2:18) {
  model_summaries <- rbind(model_summaries,
                            read.table('./summary/' %&% tissue %&% '_nested_cv_chr' %&% as.character(i) %&% '_model_summaries.txt', header = T, stringsAsFactors = F))
  tiss_summary <- rbind(tiss_summary,
                             read.table('./summary/' %&% tissue %&% '_nested_cv_chr' %&% as.character(i) %&% '_tissue_chr_summary.txt', header = T, stringsAsFactors = F))
...
conn <- dbConnect(drv = driver, './dbs/' %&% tissue %&% '_models.db')
...
# Weights Table -----
weights <- read.table('./weights/' %&% tissue %&% '_nested_cv_chr1_weights.txt', header = T,stringsAsFactors = F)
for (i in 2:18) {
  weights <- rbind(weights,
              read.table('./weights/' %&% tissue %&% '_nested_cv_chr' %&% as.character(i) %&% '_weights.txt', header = T, stringsAsFactors = F))
...
unfiltered_db <- './dbs/' %&% tissue %&% '_models.db'
filtered_db <- './dbs/' %&% tissue %&% '_models_filtered_signif.db'
driver <- dbDriver("SQLite")
