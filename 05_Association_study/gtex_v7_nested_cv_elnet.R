#! /usr/bin/env Rscript
setwd("/mnt/research/qgg/wu/YFJH/eQTL_mapping/TWAS/PredictDB/cross_validate/snp_input/")
...
get_filtered_snp_annot <- function(snp_annot_file_name) {
  snp_annot <- read.table(snp_annot_file_name, header = T, stringsAsFactors = F,check.names=F) %>%
...
get_covariates <- function(covariate_file_name, samples) {
  cov_df <- read.table(covariate_file_name, header = TRUE, stringsAsFactors = FALSE, row.names = 1,check.names=F)
...
main <- function(snp_annot_file, gene_annot_file, genotype_file, expression_file,
  ...
  tiss_chr_summ_f <- '../summary/' %&% prefix %&% '_chr' %&% chrom %&% '_tissue_chr_summary.txt'
